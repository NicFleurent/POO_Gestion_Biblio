...
Objectifs:			Créer les affichages et effectuer le programme
...
public class MainFrame implements ActionListener {
	...
	public void creerbdBase() {
		...
		try{
			...
			deuxRevue = new Revue("Hockey le magazine", new GregorianCalendar(2022,9,1), 2, "790.5", 12.95, "Bimestriel", "Volume 17 numéro 2", "2434-561X", "Sport");
			troisRevue = new Revue("Coup de pouce", new GregorianCalendar(2023,2,1), 6, "370.87", 5.99, "Mensuelle", "Vol.40 no.1", "5890-357F", "Cuisine");
			quatreRevue = new Revue("Fine woodworking", new GregorianCalendar(1994,3,1), 3, "688.546", 6.95, "Bimestriel", "No.105", "8754-9867", "Ébénisterie");
			...
			deuxJournal = new Journal("La Presse", new GregorianCalendar(2003,3,30), 1, "70.556", 0.65, "Quotidienne", "30 avril 2003", "8576-942KE", "Québec(Province)");
			troisJournal = new Journal("La Nouvelle Union", new GregorianCalendar(2023,4,15), 15, "70.576", 0.25, "Hebdomadaire", "15 mai 2023", "5196-5876", "Arthabaska/Érable");
			quatreJournal = new Journal("Le Journal de Montréal", new GregorianCalendar(1997,6,17), 1, "70.558", 0.75, "Quotidienne", "17 juillet 1997", "7548-985G", "Montréal");
			...
			deuxDvd = new Dvd("Star Wars, épisode VI : Le Retour du Jedi", new GregorianCalendar(1983,4,25), 3, "520.56", 129.99, "Science-Fiction", "978-3-11-587412-8", 135, "George Lucas");
			troisDvd = new Dvd("Les Minions", new GregorianCalendar(2015,6,10), 5, "790.867", 19.99, "Animation-Comédie", "978-4-12-876952-5", 91, "Kyle Balda");
			quatreDvd = new Dvd("Les Benchwarmers", new GregorianCalendar(2006,3,7), 1, "790.584", 4.99, "Baseball-Comédie", "978-3-43-654897-2", 82, "Dennis Dugan");
			...
			unLivre = new Livre("La pierre du destin", new GregorianCalendar(2008,8,2), 5, "C840.568", 24.95, "Édition Michel Quintin", "978-2-89435-384-4", "Fantastique", "Les messagers de gaïa", "Fredrick D'Anterny", 336);
			deuxLivre = new Livre("Les 3 mousquetaires", new GregorianCalendar(2016,7,2), 3, "840.548", 29.95, "Groupe Archambault", "622-2-091525-80", "Romans Étrangers", "non-applicable", "Alexandre Dumas", 620);
			troisLivre = new Livre("Pierre Gervais : au coeur du vestiaire", new GregorianCalendar(2022,10,18), 5, "C920.547", 29.95, "Ovations Médias", "978-2-98211-600-9", "Sports", "non-applicable", "Mathias Brunet", 292);
			...
			unDictionnaire = new Dictionnaire("Le Petit Robert de la langue française 2023", new GregorianCalendar(2022,1,1), 25, "440.589", 84.95, "Robert Ed LE", "978-2-32101-763-9", "francais");
			deuxDictionnaire = new Dictionnaire("Le Robert & Collins poche anglais : anglais-français", new GregorianCalendar(2021,6,1), 10, "440.998", 12.95, "Robert Ed LE", "978-2-32101-661-8", "anglais-français");
			troisDictionnaire = new Dictionnaire("Le Petit Larousse illustré 2023", new GregorianCalendar(2022,6,1), 25, "440.598", 56.95, "Larousse", "978-2-03593-868-8", "francais");
			quatreDictionnaire = new Dictionnaire("Dictionnaire de poche français-espagnol", new GregorianCalendar(2022,7,1), 25, "440.788", 13.95, "Larousse", "978-2-03602-187-7", "espagnole-français");
		}
	...
	//***********************************************************************************************
	//Recupère la base de donner existante
	//***********************************************************************************************
	...
	private void creerPanelAjouter() {
		...
		JLabel lblAjoutNbrCopie = new JLabel("Combien de copies possèdons-nous?");
	...
	//***********************************************************************************************
	//Ajout l'item dans la base de données
	//***********************************************************************************************
				...
				try {
					...
					if(validerExisteDeja(revue)) {
						...
						if(!modifierActif) {
							...
							bdBiblio.ajouter(revue);
							JOptionPane.showMessageDialog(null, "Le document à été modifier dans la base de données");
							supprimerDocument();
						}
					}
					else {
						JOptionPane.showMessageDialog(null, "L'item est déjà présent dans la base de données");
					}
	...
	//***********************************************************************************************
	//Créé la page Supprimer un document
	//***********************************************************************************************
	...
	//***********************************************************************************************
	//Supprime l'item dans la base de données
	//***********************************************************************************************
		...
		btnRechercherDewey.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					JOptionPane.showMessageDialog(null, "Entrer un nombre réel pour rechercher avec une cote Dewey");
				}
	...
	//***********************************************************************************************
	//Création de l'affichage de chaque item selon leur classes
	//***********************************************************************************************
	...
	private void archiverDocuments() {
		...
		if(periodeExiste || volumeExiste || multimediaExiste){
			int validation = JOptionPane.showConfirmDialog(	null, 
															"Des fichiers sont déjà présent dans l'archive, voulez-vous les écraser?",  
															"Biblio Manager Pro FR - Archiver" , 
															JOptionPane.YES_NO_OPTION  ) ;
			if (validation  ==  0) {
				creerFichierTexte();
				lblArchiver = new JLabel("Les fichiers dans l'archive ont été écrasés");
			}
			else {
				lblArchiver = new JLabel("Les fichiers n'ont pas été archivés");
			}
		}
		else {
			creerFichierTexte();
			lblArchiver = new JLabel("Les fichiers ont été créés dans l'archive");
		}
	...
	private void creerFichierTexte() {
		...
		String strDateCreation = annee + "-" + deuxChiffres.format(mois) + "-" + deuxChiffres.format(jour) + " à " + heure + ":" + minute + ":" + seconde;
	...
	//***********************************************************************************************
	//Enlève tous les panels du frame
	//***********************************************************************************************
	...
	//***********************************************************************************************
	//Valide que le mois entré est réel
	//***********************************************************************************************
	...
	//***********************************************************************************************
	//Valide que le jour entré est réel
	//***********************************************************************************************
	...
	//***********************************************************************************************
	//Valide que l'application se ferme de la bonne façon
	//***********************************************************************************************
	private void quitterApp() {
		...
		try {
			...
			out.close();
			fileOut.close();
			JOptionPane.showMessageDialog(null, "La base de donées a été mise à jour, l'appilcation va se fermer");
		} catch (IOException e) {
			e.printStackTrace();
		}
